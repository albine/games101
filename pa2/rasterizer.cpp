...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    float xmin = std::min_element(v.begin(), v.end(), [](auto & v1, auto & v2) {
        return v1.x() < v2.x();
    })->x();
    float xmax = std::max_element(v.begin(), v.end(), [](auto & v1, auto & v2) {
        return v1.x() < v2.x();
    })->x();
    float ymin = std::min_element(v.begin(), v.end(), [](auto & v1, auto & v2) {
        return v1.y() < v2.y();
    })->y();
    float ymax = std::max_element(v.begin(), v.end(), [](auto & v1, auto & v2) {
        return v1.y() < v2.y();
    })->y();
    ...
    {
        for (size_t j = btm; j <= top; j++)
        {
            Eigen::Vector3f tri[] = {
                v[0].head<3>(), v[1].head<3>(), v[2].head<3>()
            };
...
void rst::rasterizer::clear(rst::Buffers buff)
{
    ...
    if ((buff & rst::Buffers::Depth) == rst::Buffers::Depth)
    {
        ...
        std::array<float, 4> tmp{std::numeric_limits<float>::infinity(),
         std::numeric_limits<float>::infinity(), 
         std::numeric_limits<float>::infinity(), 
         std::numeric_limits<float>::infinity()};
        std::fill(msaa_buf.begin(), msaa_buf.end(), tmp);
    }
}
